function resid=fullmodel7(params,data)
% v3-Original model
...
% v7-Testing different noise parameters, see if it chagnes things
p_whole=params(1);
p_partA=params(2);
p_partB=params(2);
p_colorA=params(3);
...
if (p_colorA+p_colorB)<=1 && sum(params<0)==0 && (sum(params([1:3]))<=1)
    ...
    colorB_prob=pdfilaplace(inds,0,sd_colorB)/sum(pdfilaplace(inds,0,sd_colorB)); 
    ...
    Apart_Acolor_matrix=Apart_Acolor_matrix*(diagSum+currSum)/currSum;
    ...
    Acolor_Acolor_matrix=Acolor_Acolor_matrix*(diagSum+currSum)/currSum;
    ...
    Bcolor_Bpart_matrix=Bcolor_Bpart_matrix*(diagSum+currSum)/currSum;
    ...
    Bcolor_Bcolor_matrix=Bcolor_Bcolor_matrix*(diagSum+currSum)/currSum;
