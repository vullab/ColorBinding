...
nLL <- function(p.whole, p.part, p.color, scale){
  p.mat <- predictMat(p.whole, p.part, p.color, 0, scale, norep=1)
  # for norep only
  diag(p.mat) <- 1
  ...
  for(s in unique(tmp$subjectID)){
    ...
    if(!is.logical(res)){
      ...
      fits[[v]] <- rbind(fits[[v]], data.frame(
        subject = s,
        p.whole = logistic(C['p.whole']),
        p.part = logistic(C['p.part']),
        p.color = logistic(C['p.color']),
...
save('data', 'fits', file = 'fits.norep.Rdata')
