...
% v3v2Acr-Fit across subjects, see if gives different color estimate
close all
clear all
fclose all;
clc;
...
% 
% for fm6-totresidual=5.2156e+05
...
% for fm8 totres=6.2798e+05
% Not converging, trying to redistribute prob from diag more broadly
quickie = @(params,currCount)(fullmodel8(params,currCount));
startParam=[.1 .15 .25 .8];
lbound=[0 0 0 0 ];
ubound=[1 1 .5 inf ];
name='probSampsAcr_fm8.mat';

% for fm9 totres=5.4990e+05
% Not underestimating part binding, getting rid of redistribution and just
% normalizing
% quickie = @(params,currCount)(fullmodel9(params,currCount));
% startParam=[.1 .15 .25 .8];
% lbound=[0 0 0 0 ];
% ubound=[1 1 .5 inf ];
% name='probSampsAcr_fm9.mat';
...
for ci=1:length(stimtype)
    ...
    ps=nanmean(params(ci,:,:),3);

    [residual mllk]=fullmodel9_llk(ps,currCount);
    totresidual=totresidual+sum(residual(:));
    modllk(:,:,ci)=mllk;
...
% pWhole2=nanmedian(params(:,1,:),3);
% pPart2=nanmedian(params(:,2,:),3);
% pColor2=nanmedian(params(:,3,:),3);
% sdSamp2=nanmedian(params(:,4,:),3);
