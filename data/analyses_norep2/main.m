...
% Figure out score of each subject
score=nan(30,length(allData)); % #Subj x #TrialCorrect vs. #TrialTotal x #Conditions
...
for ci=1:length(allData)
    ...
    if ~isempty(currCond)
        for si=1:size(currCond,3)
            ...
            score(si,ci)=hits; % Number of correctly recalled whole objects
        end
    end
...
% [a b]=find(score<=prctile(score,0));
% for i=1:length(a)
%     allData{b(i)}(:,:,b(i))=[];
% 
% end
