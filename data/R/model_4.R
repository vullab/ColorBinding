...
fitBindingModel2 <- function(x,data,plot = FALSE){
  if (all(x>=0) & all(x[0:3] <=1) & all(x[7] <=1)){
    #fill out paramters
    # p_whole_item = x[1]
...
runBindingModel <- function(p_whole_item,
	...
	finalMatrix <- p_whole_item * whole_matrix + 
		...
		(1 - p_whole_item) * (
			...
			#get part A
			(p_part_A * p_part_B) * Apart_Bpart_matrix +  # TFL-Sample parts
				(1-p_part_B) * ( # TFL-If you don't remember part B then
				(p_part_A *  p_color_A) * Apart_Acolor_matrix + # TFL-Guess an A color
				(p_part_A * p_color_B) * Apart_Bcolor_matrix + # TFL-Guess a B color
				(p_part_A *  (1 - p_color_A - p_color_B)) * Apart_G_matrix ) + # TFL-Or just guess 
