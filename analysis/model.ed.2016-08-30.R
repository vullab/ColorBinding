...
predictMat <- function(p.whole, p.part, p.color, p.rep, scale, norep){
  ...
  v.part.i = p.part*c(p.positions, rep(0, 5)) + (1-p.part)*p.color*c(p.positions, p.positions) + (1-p.part)*(1-p.color)*1/10
  v.part.j = p.part*c(rep(0, 5), p.positions) + (1-p.part)*p.color*c(p.positions, p.positions) + (1-p.part)*(1-p.color)*1/10
  ...
  mat.final <- p.whole*mat.whole + 
    (1-p.whole)*(norep*mat.norep +
