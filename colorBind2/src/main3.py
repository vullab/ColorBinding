...
def oneTrial(trialdata):
    ...
    posinds=showColorOptions(colors,idx1,centerVert,trialdata['bars-or-spot'],version,offset)
    positions=posinds[0]
    indices=posinds[1]
...
def showColorOptions(colororder,idx1,centerVert,bs,version,offset):
    newcol = []
    ...
    if centerVert:
    	if bs==2:
        	...
        	drawer.drawCrossOptions2(newcol, 0, barsize,True)
        	drawer.drawCrossOptions2(newcol, 90, barsize,False)
    	...
    	if bs==2:
        	...
        	drawer.drawCrossOptions2(newcol, 90, barsize,True)
        	drawer.drawCrossOptions2(newcol, 0, barsize,False)
        elif version=='5' or version=='6':
...
def waitformouseclick(positions):
	...
	if count<5:
        ...
        for event in pygame.event.get():
            if event.type==MOUSEBUTTONUP and count==5: 
